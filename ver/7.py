...
def add_to_cart(call):
    ...
    # Получаем текущий заказ или создаем новый, если его нет
    conn = sqlite3.connect("../database.db")
    cursor = conn.cursor()
    cursor.execute("SELECT order_number FROM customers WHERE user_id = ?", (user_id,))
    order_number = cursor.fetchone()

    # Если заказа нет, создаем новый заказ
    if not order_number:
        cursor.execute("INSERT INTO customers (user_id, name, phone, order_number) VALUES (?, ?, ?, ?)",
                       (user_id, '', '', 1))
        conn.commit()
        order_number = 1
    else:
    ...
    # Добавляем новое блюдо в корзину для текущего заказа
    ...
    if dish:
        ...
        conn = sqlite3.connect("../database.db")
        cursor = conn.cursor()
        cursor.execute("""INSERT INTO cart (user_id, dish_id, quantity)
                          VALUES (?, ?, ?)""", (user_id, dish_id, quantity))
        conn.commit()
        conn.close()
...
def get_phone(message):
    ...
    order_number = cursor.fetchone()[0] + 1 if cursor.fetchone()[0] else 1
