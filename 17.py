...
# Глобальная переменная для отслеживания текущей страницы отзывов
reviews_offset = 0
...
def start(message):
    ...
    reviews_btn = types.KeyboardButton("Отзывы клиентов")
...
@bot.message_handler(func=lambda message: message.text == "Отзывы клиентов")
def show_reviews(message):
    """Отображает 5 последних отзывов"""
    global reviews_offset
    conn = sqlite3.connect("database.db")
    cursor = conn.cursor()
    cursor.execute("""
        SELECT orders.id, customers.user_id, orders.review 
        FROM orders
        JOIN customers ON orders.customer_id = customers.id
        WHERE orders.review IS NOT NULL
    ...
    if reviews:
        ...
        for review_id, user_id, review_text in reviews:
            # Получаем имя пользователя
            conn = sqlite3.connect("database.db")
            cursor = conn.cursor()
            cursor.execute("SELECT name FROM orders WHERE id = ?", (review_id,))
            name = cursor.fetchone()[0]
            conn.close()

            response += f"👤 {name}: {review_text}\n\n"

        keyboard = types.InlineKeyboardMarkup()
        keyboard.add(types.InlineKeyboardButton("Показать следующие 5 отзывов", callback_data="next_reviews"))

        bot.send_message(message.chat.id, response, parse_mode="Markdown", reply_markup=keyboard)
    else:
        bot.send_message(message.chat.id, "Нет доступных отзывов.")


@bot.callback_query_handler(func=lambda call: call.data == "next_reviews")
def next_reviews(call):
    """Отображает следующие 5 отзывов"""
    global reviews_offset
    reviews_offset += 5
...
def get_phone(message):
    ...
    if order_id:
        ...
        conn.commit()
        conn.close()
        bot.send_message(user_id, "Спасибо! Ваш заказ оформлен. Ожидайте подтверждения.")
        # Очищаем корзину
        conn = sqlite3.connect("database.db")
        cursor = conn.cursor()
        cursor.execute("DELETE FROM cart WHERE user_id = ?", (user_id,))
        ...
        bot.send_message(user_id, "Произошла ошибка при оформлении заказа. Попробуйте снова.")
