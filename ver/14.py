...
def start_payment(call):
    ...
    После ввода телефона - показываем кнопку 'Оплатить заказ #X'
    """
    user_id = call.message.chat.id
...
def process_payment(call):
    ...
    time.sleep(5)  # Можно увеличить при необходимости
...
def leave_review_handler(call):
    ...
    Просим ввести отзыв текстом. Если пользователь не хочет, пусть пишет 'нет'
    """
    user_id = call.message.chat.id
...
def process_review(message):
    ...
    # Если пользователь написал "нет", считаем, что отзыв не оставлен
