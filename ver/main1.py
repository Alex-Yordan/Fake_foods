...
def show_cart(call):
    ...
    cursor = conn.cursor()
    cursor.execute("""
        SELECT menu.name, menu.price, cart.quantity, (menu.price * cart.quantity) as total
        FROM cart
    ...
    for name, price, quantity, total in cart_items:
        response += f"{name} - {quantity} шт. (💰 {price} ₽ за шт.) = {total} ₽\n"
    response += f"\n💰 *Итого: {total_price} ₽*"
