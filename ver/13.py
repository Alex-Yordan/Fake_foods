...
""")
# Таблица orders для хранения каждого заказа
cursor.execute("""
...
def get_name(message):
    ...
    conn = sqlite3.connect("../database.db")
    cursor = conn.cursor()
    # Проверяем, существует ли запись в customers для данного user_id
...
def get_phone(message):
    ...
    conn.commit()
    conn.close()
    bot.send_message(user_id, f"Ваш заказ #{order_id} оформлен. Ожидайте подтверждения оплаты.")
    simulate_payment(user_id, order_id)


def simulate_payment(user_id, order_id):
    """Эмулирует оплату заказа и переходит к отзыву"""
    time.sleep(10)  # Эмуляция задержки оплаты
    conn = sqlite3.connect("../database.db")
    cursor = conn.cursor()
    cursor.execute("DELETE FROM cart WHERE user_id = ?", (user_id,))
    conn.commit()
