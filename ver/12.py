...
# Глобальный словарь для хранения id текущего заказа пользователя
current_order = {}

# Создание таблиц, если их нет (таблица menu не трогаем)
...
""")
# Таблица customers теперь содержит только user_id
cursor.execute("""
...
""")
# Таблица orders хранит данные по каждому заказу
cursor.execute("""
...
def start(message):
    ...
    bot.send_message(message.chat.id, "Привет! Добро пожаловать в сервис заказа еды. Нажмите 'Старт', чтобы начать.", reply_markup=keyboard)
...
@bot.callback_query_handler(func=lambda call: call.data in [c[0] for c in sqlite3.connect(
    "../database.db").cursor().execute("SELECT DISTINCT category FROM menu").fetchall()])
...
def start_payment(call):
    """Запрашивает имя и телефон для оформления заказа"""
    user_id = call.message.chat.id
...
def get_name(message):
    ...
    conn = sqlite3.connect("../database.db")
    cursor = conn.cursor()
    # Проверяем, существует ли уже запись для этого пользователя в таблице customers
...
def get_phone(message):
    ...
    bot.send_message(user_id, f"Ваш заказ #{order_id} оформлен. Ожидайте подтверждения.")
...
def process_payment(call):
    ...
    keyboard.add(types.InlineKeyboardButton("Без отзыва", callback_data="no_review"))
...
def leave_review(call):
    ...
    keyboard = types.InlineKeyboardMarkup(row_width=2)
    keyboard.add(
        types.InlineKeyboardButton("Отправить отзыв", callback_data="send_review"),
